...
export class QuickMemoStorage {
    ...
    async initialize(): Promise<void> {
        ...
        // mdファイル用のディレクトリを作成
        if (this.storageUri) {
            const mdDir = vscode.Uri.joinPath(this.storageUri, 'quickMemo');
            try {
    ...
    async addMemoToFolder(folderName: string, title: string, links: string[] = []): Promise<QuickMemoFile> {
        ...
        // mdファイルを作成
        if (this.storageUri) {
            const mdPath = vscode.Uri.joinPath(this.storageUri, 'quickMemo', fileName);
    ...
    async getMemoContent(memo: QuickMemoFile): Promise<string> {
        ...
        const mdPath = vscode.Uri.joinPath(this.storageUri, 'quickMemo', memo.file);
